...
build: generate
	@go build -ldflags "-s -w -X main.build=${gitTime}.${gitCID}" -o "./bin/raccoon"
	@echo "[OK] App binary was created!"

buildcross: generate
	@CGO_ENABLED=0 GOARCH=amd64 GOOS=linux go build -ldflags "-s -w -X main.build=${gitTime}.${gitCID}" -o "./bin/raccoon.amd64"
	@cp ./bin/raccoon.amd64 ./bin/raccoon.x86_64
	@echo "[OK] App amd64 binary was created!"
	@CGO_ENABLED=0 GOARCH=arm64 GOOS=linux go build -ldflags "-s -w -X main.build=${gitTime}.${gitCID}" -o "./bin/raccoon.arm64"
	@cp ./bin/raccoon.arm64 ./bin/raccoon.aarch64
	@echo "[OK] App arm64 binary was created!"
	@CGO_ENABLED=0 GOARCH=mips64le GOOS=linux go build -ldflags "-s -w -X main.build=${gitTime}.${gitCID}" -o "./bin/raccoon.mips64le"
